...
float RobotContext::motor_velocity(int motor_idx, float vx, float vy, float va) const
{
    switch (motor_idx) {
    case 1:
        return (2.0f * wheelbase_ * va - sqrt(3.0f) * vx + vy) / (2.0f * wheel_radius_);
    case 2:
        return (sqrt(2.0f) * wheelbase_ * va - vx - vy) / (sqrt(2.0f) * wheel_radius_);
    case 3:
        return (sqrt(2.0f) * wheelbase_ * va + vx - vy) / (sqrt(2.0f) * wheel_radius_);
    case 4:
        return (2.0f * wheelbase_ * va + sqrt(3.0f) * vx + vy) / (2.0f * wheel_radius_);
...
{
    String result = "<0," + String(rd) + "," + String(rt) + ","
                  + "1," + String(ld) + "," + String(lt) + ">";
...
bool RobotContext::send_feedback_packet()
{
    ...
    esp_err_t e = esp_now_send(mac_feedback_,
                               (uint8_t *)&data_feedback_,
