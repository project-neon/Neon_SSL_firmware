...
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len)
{
    ...
    robot->first_mark = millis();
    strcpy(robot->commands, robot->data_received.message);
    robot->new_data = true;
}
...
bool send_feedback()
{
    ...
    esp_err_t e = esp_now_send(robot->mac_feedback, (uint8_t *)&robot->data_feedback, sizeof(robot->data_feedback));
    if (e != ESP_OK)
    {
...
{
    char *strtokIndx;
    strtokIndx = strtok(robot->temp_chars, ",");

    while (strtokIndx != NULL)
    {
        robot->id = atoi(strtokIndx);

        if (robot->id == robot->robot_id)
        {
            robot->new_data = false;
            robot->stop = false;

            strtokIndx = strtok(NULL, ",");
            robot->v_l = atof(strtokIndx);
            strtokIndx = strtok(NULL, ",");
            robot->v_a = atof(strtokIndx);
            strtokIndx = strtok(NULL, ",");
            robot->th = atof(strtokIndx);
            strtokIndx = strtok(NULL, ",");
